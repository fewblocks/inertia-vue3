...
各コンポーネントは、grep をかけたときにアクションを起こしやすいように、識別子をつける
プログラミング時に絶対使わないような記号が適している。
vue では何でも使えるので、一旦
`@v overview` とかで試してみる。

各記号とその意味（必要ならどんどん足していく）

| ワード     | 概要                       |
| ---------- | -------------------------- |
| overview   | ファイルの概要             |
| complicate | 複雑なロジックや概念を内包 |
| tips       | 有効なテクニックを使用     |
| age_old    | 方式が古い                 |
| age_new    | 方式が新しい               |
| domain     | ドメイン領域の情報         |
...
今回は型ファイルに積極的に書いていく実験をしてみたい
が、自分なりのまとめ方でよくて、クラス化やオブジェクト化を意識した作りには
今回はしない

### 「overview」ファイルの概要

```js
// @v overview: コレクション対象、クイズ後にユーザーが選別する、問題文と回答が書かれたカードコンポーネント
```

### 「complicate」複雑なロジックや概念を内包

```js
// @v complicate: 難易度ごとの問題文生成機構あり
```

### 「tips」有効なテクニックを使用

```js
// @v tips: パーティクルエフェクトをCSSで表現
```

### 「age_old」方式が古い

```js
// @v age_old: 非同期通信は vue-axios を使って書くべき
```

### 「age_new」方式が新しい

```js
// @v age_new: <Suspense> は実験的な機能でstable になることは保証されていない
```
...
// @v domain:
/**
 * カードには 裏面と表面があり、表側には日本語のセリフが
 * 裏面には英訳が書いてある。
 * カードはユーザーがコレクションする対象であり、
 * そのコレクションができることを担保にユーザー登録への誘導を行う。
 */
```
